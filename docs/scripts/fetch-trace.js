...
function sanitiseSpan(s) {
  ...
  // Ensure parentID is present (root spans may omit it)
  if (!('parentID' in out)) out.parentID = '0';
  // Only include error field when it is set to 1
...
async function main() {
  ...
  if (!Array.isArray(spans)) {
    console.error('Unexpected response shape — no data.attributes.spans array.');
